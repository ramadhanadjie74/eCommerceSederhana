...
if (isset($_POST['save']))
{
	...
	$log="Menambahkan Akun Admin dengan Nama: $nama";
	...
	if ($birth > $today)
	{
		...
		echo "<script>location='media.php?halaman=tambahadmin'</script>";
	}
	else
	{
		$koneksi->query("INSERT INTO admin (namaadmin, telefonadmin, tladmin, emailadmin, alamatadmin, username, password, fotoadmin, posisiadmin) VALUES ('$nama', '$telefon', '$birth', '$email', '$alamat', '$usern', '$pass', '$foto', '$pa')");
		...
		echo "<script>alert('Akun Admin baru Berhasil Disimpan');</script>";
